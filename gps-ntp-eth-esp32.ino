...
void initGPS() {
    ...
    attachInterrupt(digitalPinToInterrupt(GPS_PPS_PIN), [](){
...
void handleNTP() {
    ...
    if (sysStatus.bits.noSatellite || sysStatus.bits.noPPS) return;
...
void sendNtpReply(const IPAddress& remoteIP, uint16_t port) {
    ...
    for (int i = 0; i < 8; ++i) packetBuffer[16 + i] = (refTs >> (56 - 8*i)) & 0xFF;

    memcpy(packetBuffer + 24, clientOrigTs, 8);

    uint64_t rxTs = toNtp64(rxTime);
    for (int i = 0; i < 8; ++i) packetBuffer[32 + i] = (rxTs >> (56 - 8*i)) & 0xFF;

    txTime = getCurrentTime();
    uint64_t txTs = toNtp64(txTime);
    for (int i = 0; i < 8; ++i) packetBuffer[40 + i] = (txTs >> (56 - 8*i)) & 0xFF;
...
#ifdef DEBUG
    Serial.print("New IP: "); Serial.println(ipAddr);
#endif
}
